...
/// # v4 or v3 lists?
/// You may have noticed that the a v4 version of our list API exists. While these v3 list methods
/// continue to work, all of the new features you can see on our website are only available if you switch to the v4 lists.
///
...
public class TMKListsAPIWrapper: TMKAPIWrapper {
    ...
    public func getDetails(forList list: Int,
                           language: String?,
                           completion: @escaping TMKObjectHandler<TMDBList>) {
    ...
    public func checkItemStatus(whetherMovie movie: Int,
                                isInList list: Int,
                                completion: @escaping TMKObjectHandler<TMDBItemStatus>) {
    ...
    public func createList(named name: String,
                           description: String,
                           language: String,
                           completion: @escaping TMKAnyHandler<Int>) {
        do {
            let list = ["name": name,
            ...
            performRequest(method: "POST",
                           ...
                           authentication: .user,
                           expectedStatusCode: 201) { (result: TMKJSONReturn) in
                switch result {
    ...
    ///   - list: List's ID
    ///   - completion: Completion handler.
    public func add(movie: Int, toList list: Int, completion: @escaping TMKHandler) {
        do {
            performRequest(method: "POST",
    ...
    ///   - list: List's ID.
    ///   - completion: Completion handler.
    public func remove(movie: Int, fromList list: Int, completion: @escaping TMKHandler) {
        do {
            performRequest(method: "POST",
    ...
    ///   - list: List's ID.
    ///   - completion: Completion handler.
    public func clear(list: Int, completion: @escaping TMKHandler) {
        performRequest(method: "POST",
                       path: "/list/\(list)/remove_item",
                       query: ["confirm": "true"],
                       authentication: .user,
    ...
    ///   - list: List's ID.
    ///   - completion: Completion handler.
    public func delete(list: Int, completion: @escaping TMKHandler) {
