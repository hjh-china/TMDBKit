...
public class TMKCertificationsAPIWrapper: TMKAPIWrapper {
    ...
    public func getMovieCertifications(completion: @escaping TMKObjectHandler<[String: [TMDBCertification]]>) {
    ...
    /// for that country.
    public func getTVCertifications(completion: @escaping TMKObjectHandler<[String: [TMDBCertification]]>) {
        certificationsHelper(1, completion: completion)
    }
    
    func certificationsHelper(_ flag: Int,
                              completion: @escaping TMKObjectHandler<[String: [TMDBCertification]]>) {
        let path = flag == 0 ? "/certification/movie/list" : "/certification/tv/list"
        performRequest(path: path) { (result: TMKObjectReturn<[String: [String: [TMDBCertification]]]>) in
            switch result {
